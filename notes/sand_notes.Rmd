...
\begin{itemize}
\item If $p = c/N_V$ and $c>1$ then whp, their will be a single connected component with $O(N_V)$ vertices (called the \textit{giant component}) and the other smaller components will only have $O(\log N_V)$. If $c<1$ then whp all components will have the smaller $O(\log N_V)$ size. 
\item In the $c>1$ case, the density of the graph will be $p\sim N_V^{-1}$ so $G$ is expected to be sparse for large $N_V$. 
\item Under the sample $p = c/N_V$ model, for large $N_V$ the degree distribution $f_d(\cdot)\sim\text{Pois}(c)$. 
\item Under this model, clustering is not expected 
\item These networks contain the \textit{small world property}: whp, the diameter varies like $O(\log N_V)$ when $N_V\to\infty$ 
\end{itemize}
...
\begin{enumerate}
\item Specify an ordered degree sequence $\{d_{(1)},\ldots, d_{(N_V)}\}$ 
\item For each vertex $i = 1,2,\ldots, N_V$, create a list containing a total of $d_{(i)}$ copies of the vertex $i$. 
\item Sample pairs from this list with equal probability
\item Remove the sampled pair for the list 
\item Iterate until the list is empty
\end{enumerate}
...
\begin{enumerate}
\item Starting from an initial degree sequence $\{d_{(1)}, \ldots, d_{(N_V)}\}$ construct an initial graph $G^{(1)}$
\item Randomly select two edge $e_1 = \{u_1, v_1\}$ and $e_2 = \{u_2, v_2\}$ 
\item Swith the edge to attain $e_1^{*} = \{u_1, v_2\}$ and $e_2^{*} = \{u_2, v_1\}$
\item Remove $e_1, e_2$ and replace with $e_1^{*}$ and $e_2^{*}$ (assuming neither exist) to attain $G^{(2)}$
\item Iterate sifficiently long to attain sample 
\end{enumerate}
